...
/* ── Shared filter panel sub-components ──────────────────────────────────── */
...
export default function CustomerRentalsPage() {
  ...
  const filtered = useMemo(() => all.filter((r: ApiBooking) => {
    if (status !== "All" && capitalize(r.status) !== status) return false;
    if (search && !carName(r).toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }), [all, status, search]);

  const totalSpent = all.filter(r => r.status === "completed")
    .reduce((s, r) => s + r.total_amount, 0);

  const hasFilter = status !== "All" || !!search || !!dateFrom || !!dateTo;
  ...
  return (
    <CustomerShell title="My Rentals" subtitle={`${all.length} rental${all.length !== 1 ? "s" : ""}`}
      actions={
        <button onClick={() => router.push("/customer/rent-car")}
          ...
          <span style={{ display: "flex" }}>{Ic.Rentals(t.accentFg)}</span>Rent a Car
        </button>
      }
    >
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ width: panelCollapsed ? 40 : 240, flexShrink: 0, background: t.cardBg, borderRight: `1px solid ${t.border}`, display: "flex", flexDirection: "column", overflow: "hidden", fontFamily: "'DM Sans',sans-serif", transition: "width 0.22s cubic-bezier(0.4,0,0.2,1)", position: "relative" }}>
          <div style={{ padding: panelCollapsed ? "13px 0 11px" : "13px 16px 11px", borderBottom: `1px solid ${t.border}`, display: "flex", alignItems: "center", justifyContent: panelCollapsed ? "center" : "space-between", flexShrink: 0, transition: "padding 0.22s" }}>
            ...
            {!panelCollapsed && hasFilter && (
              <button onClick={() => { setStatus("All"); setSearch(""); setDateFrom(""); setDateTo(""); }}
                ...
                Reset all
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 2L10 10M10 2L2 10" stroke={t.textMuted} strokeWidth="1.5" strokeLinecap="round"/></svg>
              </button>
            )}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", background: t.bg }}>
          ...
          {loading ? (
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"60%", color:t.textMuted, fontSize:13 }}>Loading rentals…</div>
          ) : all.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "60%", gap: 16 }}>
              <div style={{ width: 60, height: 60, borderRadius: 16, background: t.cardBg, border: `1px solid ${t.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ display: "flex", opacity: 0.4 }}>{Ic.Rentals(t.textMuted)}</span>
              </div>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: t.textPri, marginBottom: 6 }}>No rentals yet</div>
                <div style={{ fontSize: 12, color: t.textMuted, marginBottom: 18 }}>Browse our fleet and rent a car today.</div>
                <button onClick={() => router.push("/customer/rent-car")}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 4 }}>
                {[
                  ...
                  { label: "Pending",       value: all.filter(r => r.status === "pending").length,                     icon: (c: string) => Ic.History(c) },
                  { label: "Total Spent",   value: fmtCurrency(totalSpent),                                            icon: (c: string) => Ic.Payment(c) },
                ].map(s => (
              ...
              <div style={{ fontSize: 12, color: t.textMuted }}>
                Showing <strong style={{ color: t.textPri }}>{filtered.length}</strong> of {all.length} rentals
              </div>

              {filtered.length === 0 ? (
                <div style={{ padding: "40px", textAlign: "center", color: t.textMuted, fontSize: 13, background: t.cardBg, borderRadius: 12, border: `1px solid ${t.border}` }}>
                  No rentals match your filters.
                </div>
              ) : (
                filtered.map((r: ApiBooking) => {
                  const sc = STATUS_COLOR[capitalize(r.status)] ?? t.textMuted;
                  const days = Math.max(1, Math.ceil((new Date(r.end_date).getTime() - new Date(r.start_date).getTime()) / 86_400_000));
                  const name = carName(r);
                  return (
                    <div key={r.id}
                        ...
                        <div style={{ flex: 1, padding: "18px 20px" }}>
                          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
                            <div>
                              ...
                              <div style={{ fontSize: 10, color: t.textMuted }}>{r.car?.plate_number ?? "—"} · {r.id.slice(0,8).toUpperCase()}</div>
                            </div>
                            <div style={{ textAlign: "right" }}>
                              <div style={{ fontSize: 17, fontWeight: 800, color: t.textPri }}>{fmtCurrency(r.total_amount)}</div>
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10, paddingTop: 10, borderTop: `1px solid ${t.divider}` }}>
                            <div style={{ fontSize: 11, color: t.textMuted }}><span style={{ fontWeight: 600 }}>{days} day{days > 1 ? "s" : ""}</span> · ${r.daily_rate}/day</div>
                            <button onClick={() => router.push("/customer/rent-car")}
