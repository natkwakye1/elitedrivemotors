...
export default function CustomerPurchasesPage() {
  ...
  const filtered = useMemo(() => all.filter((p: ApiBooking) => {
    if (status !== "All" && capitalize(p.status) !== status) return false;
    if (search && !carName(p).toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }), [all, status, search]);
  ...
  return (
    <CustomerShell title="My Purchases" subtitle={loading ? "Loading…" : `${all.length} vehicle${all.length !== 1 ? "s" : ""} purchased`}
      actions={
        <button onClick={() => router.push("/customer/buy-car")}
          ...
          <span style={{ display: "flex" }}>{Ic.Buy(t.accentFg)}</span>Browse for Sale
        </button>
      }
    >
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", background: t.bg }}>
          {loading ? (
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"60%", color:t.textMuted, fontSize:13 }}>Loading purchases…</div>
          ) : all.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "60%", gap: 16 }}>
              <div style={{ width: 60, height: 60, borderRadius: 16, background: t.cardBg, border: `1px solid ${t.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ display: "flex", opacity: 0.4 }}>{Ic.Buy(t.textMuted)}</span>
              </div>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: t.textPri, marginBottom: 6 }}>No purchases yet</div>
                <div style={{ fontSize: 12, color: t.textMuted, marginBottom: 18 }}>Browse our cars for sale and find your perfect match.</div>
                <button onClick={() => router.push("/customer/buy-car")}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 4 }}>
                {[
                  ...
                  { label: "Total Spent",      value: fmtCurrency(all.reduce((s,p) => s + p.total_amount, 0)),                         icon: (c:string) => Ic.Payment(c) },
                ].map(s => (
              ...
              <div style={{ fontSize: 12, color: t.textMuted }}>
                Showing <strong style={{ color: t.textPri }}>{filtered.length}</strong> of {all.length} purchases
              </div>

              {filtered.length === 0 ? (
                <div style={{ padding: "40px", textAlign: "center", color: t.textMuted, fontSize: 13, background: t.cardBg, borderRadius: 12, border: `1px solid ${t.border}` }}>No purchases match your filters.</div>
              ) : (
                filtered.map((p: ApiBooking) => {
                  const sc = STATUS_COLOR[capitalize(p.status)] ?? t.textMuted;
                  const name = carName(p);
                  return (
                    <div key={p.id}
                        ...
                        <div style={{ flex: 1, padding: "20px 22px" }}>
                          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
                            <div>
                              ...
                              <div style={{ fontSize: 11, color: t.textMuted }}>{p.id.slice(0,8).toUpperCase()} · Purchased {fmtDate(p.created_at)}</div>
                            </div>
                            <div style={{ textAlign: "right" }}>
                              <div style={{ fontSize: 20, fontWeight: 800, color: t.textPri }}>{fmtCurrency(p.total_amount)}</div>
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 12, paddingTop: 12, borderTop: `1px solid ${t.divider}` }}>
                            <div style={{ fontSize: 11, color: t.textMuted }}><span style={{ fontWeight: 600 }}>Amount:</span> {fmtCurrency(p.total_amount)}</div>
                            <button onClick={() => router.push("/customer/buy-car")}
